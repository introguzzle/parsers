...
public class JSONMapperImpl implements JSONMapper {
    ...
    public JSONMapperImpl(FieldNameConverter<Field> nameConverter) {
        this.nameConverter = nameConverter;
    }

    @Override
    public JSONMapper bindTo(Class<? extends Bindable> type) throws BindException {
        Binder binder = new ByteBinder(this, type);
        binder.inject(type);
        return this;
    }
    ...
    private MethodHandle getGetter(Field field) {
        return GETTER_CACHE.computeIfAbsent(field, f -> {
            try {
                ...
                throw new MappingException("Cannot retrieve value", e);
            }
        });
    }
    ...
    public <T> TypeHandler<T> getTypeHandler(Class<T> type) {
        return (TypeHandler<T>) typeHandlerCache.computeIfAbsent(type, this::findMostSpecificHandler);
    }
    ...
    private Object map(@Nullable Object object, MappingContext context) {
        ...
        List<Field> fields = getFieldAccessor().get(object.getClass());
        ...
        for (Field field : fields) {
            ...
            try {
                value = getGetter(field).invokeWithArguments(object);
            } catch (Throwable e) {
